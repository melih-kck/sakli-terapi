# Teslim Hazırlığı

Son güncelleme: 28 Temmuz 2026

## Profesyonel Portföy Sürümü

- [x] Uygulamanın varsayılan modu güvenli portföy demosu yapıldı
- [x] Danışan, uzman ve yönetici için tek tıkla kurgusal rol deneyimleri hazırlandı
- [x] Gerçek kayıt, başvuru, randevu, ödeme ve sağlık hizmeti özellikleri kapatıldı
- [x] Demo verilerinin Supabase ve Sentry'ye gönderilmesi engellendi
- [x] Kurgusal uzman kataloğu, randevu, seans ve belge onay akışları hazırlandı
- [x] Herkese açık GitHub portföy deposu, CI ve Vercel dağıtım yapısı hazır
- [x] Otomatik lint, test, build ve yüksek önem düzeyi bağımlılık denetimi var
- [x] Kritik demo akışları Chromium, Firefox ve WebKit masaüstü/mobil profillerinde uçtan uca otomatik test ediliyor
- [x] Kritik ekranlar WCAG A/AA kuralları için masaüstü ve mobilde otomatik taranıyor
- [x] Azaltılmış hareket tercihi ve erişilebilir renk kontrastları destekleniyor
- [x] Masaüstü ve mobil görünüm kabul kontrolü yapıldı

## Hazır Teknik Altyapı

- [x] Supabase Auth, rol ayrımı ve RLS güvenlik sınırları
- [x] Danışan, psikolog ve yönetici temel akışları
- [x] Metin, ses ve bulanık görüntülü seans prototipi
- [x] Bildirimler, yönetici işlem geçmişi ve Sentry entegrasyonu
- [x] Özel psikolog belge deposu ve yönetici doğrulama akışı
- [x] Belgesiz psikolog profilinin etkinleştirilmesini engelleyen veritabanı kuralı
- [x] Yönetici TOTP/MFA ve güvenli herkese açık katalog görünümü
- [x] Yedekleme, geri dönüş, olay müdahalesi ve gerçek cihaz test prosedürleri

## Canlı Hizmet Öncesi İnsan Onayı

- [ ] Gizlilik politikası, aydınlatma/açık rıza metni ve kullanım koşulları hukuk danışmanı tarafından onaylandı
- [ ] Platformun hukuki rolü ve sağlık hizmeti sunum modeli uzman görüşüyle kesinleştirildi
- [ ] Veri sorumlusu, veri saklama ve hesap silme süreleri yazılı kararlaştırıldı
- [ ] Destek, olay müdahalesi ve psikolog belge inceleme sorumluları belirlendi
- [ ] Gerçek psikologların mesleki yetkileri ve çalışma biçimleri doğrulandı
- [ ] Özel alan adı, destek kutuları ve Resend gönderici alan adı doğrulandı
- [ ] Kapalı pilot katılımcıları, kapsamı ve geri bildirim süreci belirlendi

## Bilinçli Olarak Ertelenenler

- [ ] Ödeme sağlayıcısı sözleşmesi, ödeme akışı, iade ve mutabakat
- [ ] Açık kullanıcı kaydı ve gerçek profesyonel başvuruları
- [ ] Gerçek randevu ve gerçek uzaktan görüşme hizmeti
- [ ] Geniş kitleye açık ürün duyurusu

## Teslim Kararı

Mevcut sürüm işe alım ekiplerine, ürün ve mühendislik incelemelerine sunulabilecek etkileşimli portföy demosudur. Sağlık hizmeti veya açık pilot değildir. `demo` modu ve özellik kapıları, yukarıdaki üretim gereklilikleri tamamlanana kadar korunmalıdır.
